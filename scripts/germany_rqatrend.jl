...
#    year = parsedargs["year"]
    ...
    #subcube = cubevh[X=(cubevh.X[1],cubevh.X[s]), Y=(cubevh.Y[1], cubevh.Y[s])]
                for y in  [2018,2019,2020,2016,2017,2021]

                    #@show tax
                    #@show size(tcube)
    #@time cubevh[:,:,:];
    #@time cubevh[:,:,:];
                    path = "/eodc/private/pangeojulia/germany/$(tile)_rqatrend_$(pol)_$(orbit)_thresh_$(thresh)_year_$(y)"

                    if ispath(path*".done") || ispath(path * ".nc")
                        println("Skip Computation for $path")
                        continue
                    else
                        println("Doing the RQA computation:")

                        tcube = cubevh[Time=Between(Date(y-1, 7,1), Date(y+1,7,1))]
        ...
        #redirect_stdio(stdout="stdout"*stdiopath, stderr="stderr" * stdiopath) do
            ...
            #@show nw, nt
                        ...
                        t = now()
                        println(metafile, "Processing time of $pol, $orbit, $tile, $y: ",t-s)
                        ...
                        println("End of the processing of $pol, $orbit, $tile, $y: ",now())
                 end
            end
        end
    end
